...
export default async function PinjamanProduktif() {
  ...
  if (session?.user && session.user.id) {
    const [pinjamanJasa, maxPinjaman] = await Promise.all([
      getListPinjamanById(session?.user.id, "JASA"),
      getMaxPinjamanById(session.user.id, "JASA"),
    ]);

    if (!pinjamanJasa.ok || !maxPinjaman.ok) {
      return (
        <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
          <div className="text-xl font-medium">Pinjaman Produktif</div>
          <FromStatus
            status={false}
    ...
    return (
      <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        ...
        {pendingPinjaman && (
          <TableWrapping
            header="Pengajuan Pinjaman Produktif"
            description="Data pengajuan pinjaman menunggu 3-5 hari kerja untuk persetujuan pinjaman"
        ...
        {approvedPinjaman && (
          <TableWrapping
            header="Pinjaman Produktif yang Sedang Berjalan"
            description="Data pinjaman produktif yang telah di setujui, saat ini sedang proses pelunasan"
            searchBy="tujuanPinjaman"
            labelSearch="tujuan pinjaman"
        ...
        {completedPinjaman && (
          <TableWrapping
            header="Riwayat Pinjaman Produktif"
            description="Data pinjaman produktif yang sudah selesai, telah menyelesaikan seluruh kewajiban pembayaran sesuai dengan perjanjian pinjaman"
            searchBy="noPinjaman"
