...
import StrukAsuransiHeader from "./struk-asuransi-header";

interface IStrukPinjaman {
  data: TStrukPinjaman;
}

export default function PernyataanPinjaman({ data }: IStrukPinjaman) {
  const calonPeserta = [
    {
      field: "Nama Lengkap",
    ...
    },
    {
      ...
      value: `${data.anggota.tempatLahir} - ${formatDatebyMonth(
        new Date(data.anggota.tanggalLahir),
      )}`,
    },
    {
  ...
  return (
    <div className="mt-6 grid w-full place-items-center bg-white font-serif">
      <div className="flex w-[794px] flex-col gap-1 p-2 text-sm">
        <StrukAsuransiHeader />
        <div className="my-5 w-[85%] self-center">
          <ol className="list-decimal text-left">
              ...
              <tbody>
                {calonPeserta.map((data, index) => (
                  <tr key={index} className="border border-black">
                    <td className="w-40 p-1 pb-1">{data.field}</td>
            ...
            <table className="mb-5 w-full">
              <tbody>
                {permintaanAsuransi.map((data, index) => (
                  <tr key={index} className="border border-black">
                    <td className="w-40 p-1">{data.field}</td>
                    <td className="w-2 p-1">:</td>
                    <td className="p-1">{data.value}</td>
                  </tr>
                ))}
              ...
              <tbody>
                {kesehatan.map((data, index) => (
                  <tr key={index}>
                    ...
                    <td className="border border-black p-1">{data.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <ol className="list-disc pl-4 text-justify">
              <li className="mb-1">
                Pernyataan ini saya buat sesuai dengan keadaan yang sebenarnya
                dan saya menyatakan jika ada keterangan yang tidak benar maka
                AJB Bumiputera 1912 berhak membatalkan perjanjian asuransi saya
                dan dibebaskan dari kewajiban membayar apapun
              </li>
              <li className="mb-1">
                Saya menyetujui dan memberikan izin kepada AJB Bumiputera 1912
                untuk dapat meminta data riwayat kesehatan saya di BPJS
                Kesehatan / rumah sakit / klinik / puskesmas / dokter / intansi
                kesehatan lainnya berdasarkan UU Kesehatan No 36 Thn 2009 Pasal
                57 ayat 2
              </li>
              <li className="mb-1">
          ...
          <table className="my-5 mb-5 w-full text-center font-bold ">
            <thead>
              <tr>
                <td className="w-52 border border-black p-1">
            ...
            <tbody>
              <tr>
                <td className="h-32 w-52 border border-black p-1"></td>
                <td className="h-32 w-52 border border-black p-1"></td>
              </tr>
              <tr>
                <td className="w-52 border border-black p-1"></td>
                <td className="w-52 border border-black p-1">
                  {data.anggota.nama}
                </td>
              </tr>
